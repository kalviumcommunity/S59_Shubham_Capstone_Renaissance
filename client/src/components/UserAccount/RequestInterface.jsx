...
function RequestInterface() {
    ...
    return (
        <div className="py-8 px-10 bg-gray-100">
            <h1 className="text-3xl text-slate-700 font-bold">Merge Request(s)</h1>
            <p className="text-[13.5px] mb-5 mt-1.5 text-slate-600 mt-1.5">Review the contributed material here. Review, discuss and Merge requests.</p>
            <div>
                <div className="bg-white rounded px-10 py-8 shadow border border-gray-300">
                    <h1 className="text-3xl text-slate-700 font-bold">{request && request.projectName}</h1>
                    <p className="text-slate-500 text-sm mb-5">Made on <span>{request && getDateFromISO(request.timestamp)}</span></p>
                    <div className="flex items-center">
                        <img src={imgURL} alt="profile-image" className="w-16 rounded-full" />
                        <h1 className="text-lg text-slate-700 font-bold ml-3"><span className="text-[#3F5F4F]">{request && request.contributerName}</span> wants to merge the request</h1>
                    </div>
                    <hr className="my-5" />
                    <div className="text-[14px] mt-5 pl-0 pr-0 pb-3 pt-0 bg-white rounded border border-gray-300">
                        <div className="bg-[#abe0ba] text-slate-600 m-0 py-2 pr-3">
                            <span className="font-semibold text-black ml-5">{request && request.contributerName}</span> requested
                        </div>
                        <div className="py-5 px-5 text-slate-700">
                            {request && ReactHtmlParser(request.message)}
                        </div>
                    </div>
                    <p className="text-[13.5px] text-lg mt-8 font-bold">Review the contribution:</p>
                    <div className="text-[14px] mt-5 pl-0 pr-0 pb-3 pt-0 bg-white rounded border border-gray-300">
                        <div className="bg-[#abe0ba] text-slate-600 m-0 py-2 pr-3">
